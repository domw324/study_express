...
## 더 공부할 만한 것
### 보안
여전히 보안은 취약하다. 보안에 관련해서는 더 많은 관심을 가질 필요성가 있다.

#### https만 사용
위에서 언급했지만, http 접근 보다는 **https** 접근이 보안성이 조금은 더 좋다. 따라서 https로만 접근하도록 해보자.

세션값에 secure: true 속성을 살짝 끼워넣어주자.
```javascript
app.use(session({
  secure: true,
  //... 
...
```
이렇게 되면 사용자는 https를 통해서만 웹으로 접근할 수 있다.

#### javasript 접근 불가능하게
(나쁜 의도를 가진)사용자가 입력값으로 스크립트 코드를 집어넣는다면, 웹 내의 정보를 마음대로 들여다 볼 수도 있게 된다. 따라서 세션값에 **HttpOnly** 속성을 살짝 끼워넣어주자.
```javascript
app.use(session({
  HttpOnly: true,
  //... 
...
```
그러면 script 형식으로는 접근을 할 수 없게 된다.

### 다중 사용자 관리
사용자들이 늘어나면 어플리케이션은 그만큼 복잡해지고 제공해줘야 할 서비스가 많아진다. (ex. 회원의 인증, 아이디/비밀번호 찾기, 회원정보 검색/수정 등...)

### 타사 인증 (federation authentication)
구글, 페이스북, 트위터와 같은 회사의 회원정보를 통해 회원인증을 하는 방식. 고도의 주의를 요하는 회원 관리는 타사에게 맞기고 회원의 식별 코드만을 이용하여 회원 인증 하는 방식. (+ 회원가입의 간편함)

**oauth**를 공부해보도록 하자. express에서는  **passport**라는 기능을 제공하는데, 이것이 정말 쉽게 타사 인증을 가능하게 만들어준다!